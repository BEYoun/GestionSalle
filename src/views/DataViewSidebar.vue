...
<template>
  ...
  >
    <div class="mt-6 flex items-center justify-between px-6">
      <h4>{{ Object.entries(this.data).length === 0 ? "ADD NEW" : "UPDATE" }} Selection</h4>
    ...
    <VuePerfectScrollbar class="scroll-area--data-list-add-new" :settings="settings" :key="$vs.rtl">
      <div class="p-6">
        ...
        <!-- CATEGORY -->
        <vs-select v-model="dataCategory" label="Salles" class="mt-5 w-full" name="item-category">
          <vs-select-item
            :key="item.Salles"
            :value="item.value"
            :text="item.text"
      ...
      this.$validator.validateAll().then(result => {
        if (result) {
          ...
          if (this.dataId !== null && this.dataId >= 0) {
            this.$store.dispatch("event/updateItem", obj).catch(err => {
              console.error(err);
            });
          } else {
            delete obj.id;
            obj.popularity = 0;
            this.$store.dispatch("event/addItem", obj).catch(err => {
              console.error(err);
            });
          }
